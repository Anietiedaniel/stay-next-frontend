...
const tabs = [
  ...
  { key: "admins", label: "Manage Admins", icon: "fa-user-shield" },
  { key: "agents", label: "Manage Agents", icon: "fa-user-tie" },
  { key: "visitors", label: "Manage Visitors", icon: "fa-user-friends" },
  { key: "handymen", label: "Manage Handymen", icon: "fa-tools" },
        ...
        {tabs.map((item) => (
          <li key={item.key} onClick={() => setMenuOpen(false)}>
            {item.key === "logout" ? (
              ...
              <NavLink
                to={`/super-admin-dashboard/${item.key}`}
                className={({ isActive }) => {
