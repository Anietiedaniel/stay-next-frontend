...
const AgentReferralPage = () => {
  ...
  const fetchReferralCode = async () => {
    if (!userId) return;
    try {
      const res = await AGENTAPI.get("/agents/profile/code", { headers: { "x-user-id": userId } });
  ...
  const fetchDashboard = async () => {
    if (!userId) return;
    try {
      const res = await AGENTAPI.get("/agents/profile/referraldata", { headers: { "x-user-id": userId } });
