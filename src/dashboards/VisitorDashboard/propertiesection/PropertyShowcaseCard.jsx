...
function PropertyCard({ property }) {
  ...
  return (
    <>
      <div className="relative flex flex-col md:flex-row w-full max-w-4xl items-start gap-6 border rounded-xl bg-white shadow-lg overflow-hidden dark:bg-gray-800 dark:text-gray-100">
        {/* Transaction Type Label */}
        {transactionType && (
          <div
            className={`absolute top-4 left-64 md:left-4 z-30 flex items-center gap-2 text-white text-sm font-extrabold tracking-wide 
              px-2 md:px-3 py-1 rounded-full shadow-2xl ring-2 ring-white/40 backdrop-blur-md animate-pulse-slow
              ${
          ...
          >
            <i
              className={`fas fa-tag text-white text-base hidden ${
                transactionType?.toLowerCase() === "sale"
            ...
            ></i>
            <span className="text-xs">FOR {transactionType?.toUpperCase()}</span>
          </div>
        )}

        {/* Slider */}
        <div className="relative w-full md:w-1/2 overflow-hidden h-72 group border rounded-2xl md:rounded-tl-xl md:rounded-bl-xl dark:text-gray-100">
          <div
            ref={sliderRef}
  ...
  <div key={index} className="w-full flex-shrink-0 h-72 relative">
    {slide.type === "video" ? (
      ...
      />
          ...
          {agent?.profileImage && (
            <div
              className="absolute bottom-3 left-3 flex flex-col items-center z-30 cursor-pointer dark:text-gray-100"
              title={`View all listings by ${agent.name}`}
              onClick={() => navigate(`/visitor-dashboard/agents/${agent._id}/listings`)}
            >
              <img
        ...
        {/* Details */}
        <div className="w-full md:w-1/2 px-4 py-4 text-black rounded-lg space-y-4 dark:text-gray-100">
          <div className="flex justify-between items-center dark:text-gray-100">
            <div className="text-2xl font-semibold text-gray-800 truncate dark:text-gray-100">{title}</div>
            <div className="text-xl font-bold text-black dark:text-gray-100">₦{formatPrice(price)}</div>
          </div>

          {/* Features */}
          <div className="flex flex-wrap items-center text-md text-gray-700 gap-2 font-bold dark:text-gray-100">
            <span className="flex items-center gap-1 dark:text-gray-100">
              <i className={isLand ? "dark:text-gray-100 fas fa-tree" : " dark:text-gray-100fas fa-home"}></i> {type}
            </span>
            <span>|</span>
            {!isLand && (
              <>
                <span className="flex items-center gap-1 dark:text-gray-100">
                  <i className="dark:text-gray-100 fas fa-bed"></i> {bedrooms} Bed
                </span>
                <span>|</span>
                <span className="dark:text-gray-100 flex items-center gap-1">
                  <i className="dark:text-gray-100 fas fa-bath"></i> {toilets} Bath
                </span>
              </>
            )}
            {isLand && (
              <span className="flex items-center gap-1">
                <i className="dark:text-gray-100 fas fa-ruler-combined"></i> {area}
              </span>
            )}
          </div>
          ...
          <div className="text-sm text-gray-800 dark:text-gray-100">
            <i className="fas fa-map-marker-alt text-green-500 mr-2 dark:text-gray-100"></i>
            <a
      ...
      <div className="w-full flex justify-center md:-mt-3 md:mb-10">
        <button
          onClick={() => navigate(`/visitor-dashboard/properties/${property._id}`)}
